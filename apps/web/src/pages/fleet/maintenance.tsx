...
export function MaintenancePage() {
      ...
      cell: (row) => (
        <div className="min-w-0">
          ...
          <p className="text-xs text-muted-foreground">
            {row.registrationNumber} · {humanizeEnum(row.type)}
          </p>
        </div>
      ),
    },
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="Maintenance"
        description="Scheduled work, service history and rule-based risk."
      />
            ...
            {atRisk.slice(0, 6).map((entry) => (
              <div key={entry.truckId} className="rounded-lg border border-border p-3">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Badge variant={entry.level === 'HIGH' ? 'destructive' : 'warning'} size="sm">
                    {entry.riskScore}/100
                  </Badge>
                </div>
                <Progress
                  ...
                  indicatorClassName={entry.level === 'HIGH' ? 'bg-destructive' : 'bg-warning'}
                />
