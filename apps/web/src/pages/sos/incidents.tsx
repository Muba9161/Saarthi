...
export function SosIncidentsPage() {
      ...
      cell: (row) => (
        <div className="min-w-0">
          ...
          <p className="truncate text-xs text-muted-foreground">
            {relativeTimeFrom(row.triggeredAt)}
          </p>
        </div>
      ),
    },
    ...
    { key: 'status', header: 'Status', cell: (row) => <StatusBadge status={row.status} /> },
    {
      ...
      cell: (row) => (
        <div className="min-w-0 text-sm">
          <p className="truncate">{row.driver?.name ?? 'Unknown'}</p>
          <p className="truncate text-xs text-muted-foreground">
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-sm">
          {row.acknowledgedCount}/{row.responderCount}
        </span>
      ),
    },
    {
      ...
      cell: (row) => (
        <span className="truncate text-sm text-muted-foreground">
          {row.address ?? `${row.latitude.toFixed(3)}, ${row.longitude.toFixed(3)}`}
        </span>
      ),
    },
  ];
  ...
  return (
    <div className="space-y-5">
      <PageHeader
        title="SOS incidents"
        description="Emergency events raised by drivers, and how the network responded."
      />
      <DataView
