...
/**
 * Connecting the OBD adapter, on a driver's own phone.
 *
 * The fitted tablet does this from its diagnostics screen, which a driver's
 * phone does not have and should not. What a driver needs is narrower: the
 * adapter they were handed with the keys, plugged in, paired in Android's
 * Bluetooth settings, and chosen here once.
 *
 * Once it is answering there is nothing more to do and no way to undo it. The
 * adapter is the only source of real engine data, and a driver who could switch
 * it off mid-shift would leave the fleet with a journey that quietly stopped
 * reporting everything the GPS cannot see — which is most of what a fleet
 * actually wants to know.
 */
@Composable
fun AdapterSheet(viewModel: TerminalViewModel, onClose: () -> Unit) {
    val telemetry by viewModel.uiState.collectAsState()
    val adapters by viewModel.obdAdapters.collectAsState()
    val connected = telemetry.telemetry.isReadingFromObd

    LaunchedEffect(Unit) { viewModel.refreshObdAdapters() }

    Sheet("Vehicle adapter", onClose) {
        if (connected) {
            Text(
                "Connected. Saarthi is reading this vehicle's engine directly.",
                style = MaterialTheme.typography.bodyLarge,
            )
            Spacer(Modifier.height(8.dp))
            Text(
                "It will reconnect on its own each time you start the vehicle. There is " +
        ...
        if (adapters.isEmpty()) {
            Text(
                "No paired adapters found. Pair the adapter in Android's Bluetooth " +
                    "settings first, then come back.",
                style = MaterialTheme.typography.bodyMedium,
                color = MaterialTheme.colorScheme.onSurfaceVariant,
            )
            Spacer(Modifier.height(Gutter))
            Button(onClick = { viewModel.refreshObdAdapters() }, modifier = Modifier.fillMaxWidth()) {
                Text("Look again")
            }
            return@Sheet
        }
        ...
        LazyColumn(verticalArrangement = Arrangement.spacedBy(10.dp)) {
            items(adapters, key = { it.address }) { adapter ->
                GlassCard(
                    Modifier
                        .fillMaxWidth()
                        .clickable {
                            viewModel.connectObd(adapter)
                            onClose()
                        },
                ) {
                    Text(adapter.name, style = MaterialTheme.typography.titleMedium)
                    Text(
                        adapter.address,
                        style = MaterialTheme.typography.bodyMedium,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
