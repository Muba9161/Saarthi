package com.saarthi.core.telemetry
...
data class TelemetrySnapshot(
    ...
    /**
     * Whether anything here came from the vehicle's own ECU.
     *
     * Asked by the cockpit to decide whether the adapter still needs
     * connecting. Derived from the readings rather than from a connection flag
     * on purpose: a Bluetooth link that is up but answering nothing is not an
     * adapter that is working, and it is the readings a driver is being told
     * about.
     */
    val isReadingFromObd: Boolean
        get() = values.values.any { it.source == MetricSource.OBD }
