...
class SaarthiApi(
    ...
    suspend fun checkForUpdate(): UpdateOfferDto? = getNullable(
        "/api/v1/device-gateway/terminal/update" +
            "?versionCode=${CoreConfig.versionCode}&sdk=${Build.VERSION.SDK_INT}" +
            // The app names itself. Nothing on the server can tell a driver's
            // phone from a fitted tablet — both authenticate as the same kind of
            // device — and a phone handed the tablet's build would have its
            // sign-in replaced by a kiosk.
            "&applicationId=${CoreConfig.applicationId}",
    ...
    suspend fun downloadUpdate(
        ...
        val request = Request.Builder()
            .url(
                "$baseUrl/api/v1/device-gateway/terminal/update/$versionCode/download" +
                    "?applicationId=${CoreConfig.applicationId}",
            )
            .get()
            .header("Authorization", "Bearer ${bearer()}")
