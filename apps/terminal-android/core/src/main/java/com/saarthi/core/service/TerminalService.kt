package com.saarthi.core.service
...
class TerminalService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        /*
         * Refuse to start rather than crash.
         *
         * From Android 14 a foreground service declared `location` may only be
         * started while the app actually holds a location permission, and the
         * platform enforces that by throwing `SecurityException` out of
         * `startForeground` — which, in a service, means the process dies. With
         * START_STICKY the system then restarts it, and it dies again: a crash
         * loop that looks to the person holding the phone like an app that will
         * not open.
         *
         * It is reachable on any ordinary path: a driver who declines the
         * permission prompt, a phone that reboots and fires BOOT_COMPLETED
         * before anybody has granted anything, or a driver who revokes location
         * in Settings weeks later.
         *
         * Stopping quietly is the honest response. There is nothing to report
         * without a position, and the cockpit already says so.
         */
        if (!hasLocationPermission()) {
            DebugLog.warn(
                TAG,
                "Not starting: no location permission, so there is nothing to report",
            )
            stopSelf()
            return START_NOT_STICKY
        }
    ...
    /** Either precision, because the coarse grant is a real answer too. */
    ...
    private fun buildNotification(): Notification {
        ...
        val open = PendingIntent.getActivity(
            this,
            0,
            Intent(this, app.mainActivity).apply {
        ...
        return NotificationCompat.Builder(this, SaarthiApp.SERVICE_CHANNEL_ID)
